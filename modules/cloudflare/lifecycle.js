...
export async function uploadCert(ctx, args) {
  const { certContent } = args || {};
  if (!certContent) return { ok: false, message: '凭证内容为空' };
  
  // 针对性校验
  const trimmed = certContent.trim();
  if (!trimmed.includes('-----BEGIN ARGO TUNNEL TOKEN-----') || !trimmed.includes('-----END ARGO TUNNEL TOKEN-----')) {
    return { ok: false, message: '凭证格式错误：未检测到 ARGO TUNNEL TOKEN 标识。请确保上传的是有效的 cert.pem 文件。' };
  }

  const certPath = path.join(ctx.paths.appDir, 'data', 'cert.pem');
  fs.writeFileSync(certPath, trimmed, 'utf8');
  return { ok: true, message: '凭证上传成功' };
}
